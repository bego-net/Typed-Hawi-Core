        ...
        <nav className="-mb-6 flex justify-center gap-x-12" aria-label="Footer">
          {navigation.main.map((item) => (
            <div key={item.name} className="pb-6">
              <Link to={item.href} className="text-sm leading-6 text-slate-600 hover:text-rose-600 dark:text-slate-400 dark:hover:text-white transition-colors duration-300">
                {item.name}
              </Link>
            </div>
          ))}
        </nav>
        <p className="mt-16 text-center text-xs leading-5 text-slate-500 dark:text-slate-500 transition-colors duration-500">
